...
public class SurfaceOreGen extends WorldGenerator
{
    ...
    public SurfaceOreGen(int id, int meta, int number)
    {
        this(id, meta, number, Block.grass.blockID, Block.dirt.blockID, Block.waterStill.blockID, Block.sand.blockID, Block.gravel.blockID, Block.snow.blockID);
    }

    public SurfaceOreGen(int id, int meta, int number, int... target)
    {
    ...
    public boolean generate(World world, Random random, int startX, int startY, int startZ)
    {
        ...
        int blockNumber = numberOfBlocks / 2 + random.nextInt(numberOfBlocks/2);
                    ...
                    for (int l2 = j1; l2 <= i2; ++l2)
                    {
                        ...
                        if (d12 * d12 + d13 * d13 < 1.0D)
                        {
                            for (int i3 = k1; i3 <= j2; ++i3)
                            {
                                ...
                                if (d12 * d12 + d13 * d13 + d14 * d14 < 1.0D)// && (block != null && block.isGenMineableReplaceable(par1World, k2, l2, i3, replaceBlocks)))
                                {
