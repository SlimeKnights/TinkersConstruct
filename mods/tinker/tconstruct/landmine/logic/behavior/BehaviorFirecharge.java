...
public class BehaviorFirecharge extends Behavior {
	...
	public void executeLogic(World par1World, int par2, int par3, int par4, ItemStack par5ItemStack, Entity triggerer, boolean willBlockBeRemoved) {
		...
		for (int n = 0; n < par5ItemStack.stackSize; n++) {
			triggerer.attackEntityFrom(DamageSource.generic, (int) 9F);
			triggerer.setFire(4);
			...
			for (int i = par2 - 2; i <= (par2 + 2); i++) {
				for (int j = par4 - 2; j <= (par4 + 2); j++) {
					if ((par1World.getBlockId(i, par3, j) == 0 || (Block.blocksList[par1World.getBlockId(i, par3, j)] != null && Block.blocksList[par1World.getBlockId(i, par3, j)].isBlockReplaceable(par1World, i, par3, j))) && ItemBlockLandmine.getRandom().nextInt(5) == 0 && Block.fire.canPlaceBlockAt(par1World, i, par3, j)) {
