...
public class ArrowRender extends Render
{
    ...
    public void doRenderItem (ArrowEntity arrow, double par2, double par4, double par6, float par8, float par9)
    {
        ...
        {
            GL11.glPushMatrix();
            float f2 = shouldBob() ? MathHelper.sin(((float) 0 + par9 * 0.2f) / 10.0F + 0) * 0.1F + 0.1F : 0F;
     ...
     */
    private void renderDroppedItem (ArrowEntity arrow, Icon par2Icon, int par3, float par4, float par5, float par6, float par7)
    {
        Tessellator tessellator = Tessellator.instance;

        if (par2Icon == null)
        {
            par2Icon = this.renderManager.renderEngine.getMissingIcon(arrow.getEntityItem().getItemSpriteNumber());
        }
