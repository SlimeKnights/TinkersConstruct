...
public class PartChestContainer extends TinkerStationContainer<PartChestTileEntity> {
  ...
  public PartChestContainer(int id, PlayerInventory inv, PartChestTileEntity tileEntity) {
    super(TableContainerTypes.part_chest.get(), id, inv, tileEntity);

    this.inventory = new PartChestContainer.DynamicChestInventory(TableContainerTypes.part_chest.get(), windowId, inv, tile, 8, 18, 8); // columns don't matter since they get set by gui
