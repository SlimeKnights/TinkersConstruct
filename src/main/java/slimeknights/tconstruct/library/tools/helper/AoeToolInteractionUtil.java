...
public class AoeToolInteractionUtil {
  ...
  private static boolean canBreakExtraBlock(ItemStack stack, World world, PlayerEntity player, BlockPos pos, BlockPos refPos) {
    ...
    if (player.abilities.isCreativeMode) {
      ...
      if (block.removedByPlayer(blockState, world, pos, player, false, ifluidstate)) {
  ...
  public static void breakExtraBlock(ItemStack stack, World world, PlayerEntity player, BlockPos pos, BlockPos refPos) {
    ...
    if (!world.isRemote) {
      ...
      if (block.removedByPlayer(state, world, pos, player, true, ifluidstate)) { // boolean is if block can be harvested, checked above
    ...
    // client sided handling
      ...
      if (block.removedByPlayer(state, world, pos, player, true, ifluidstate)) {
        block.onPlayerDestroy(world, pos, state);
      }
