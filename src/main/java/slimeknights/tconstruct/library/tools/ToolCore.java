...
public abstract class ToolCore extends Item implements ITinkerable, IModifiable, IToolStationDisplay, IRepairable {
  ...
  protected int calculateRepair(ItemStack tool, int amount) {
    ...
    int modifiersFree = ToolData.from(tool).getStats().freeUpgradeSlots;
    float mods = 1.0f;
  ...
  public Multimap<String, AttributeModifier> getAttributeModifiers(EquipmentSlotType slot, ItemStack stack) {
    ...
    //TinkerUtil.getTraitsOrdered(stack).forEach(trait -> trait.getAttributeModifiers(slot, stack, multimap)); TODO FIX
  ...
  public List<ITextComponent> getInformation(ItemStack stack, boolean detailed) {
    ...
    if (ToolData.from(stack).getStats().freeUpgradeSlots > 0) {
  ...
  public ItemStack buildToolForRendering() {
    if (MaterialRegistry.initialized()) {
      ...
      NonNullList<ItemStack> stacks = NonNullList.withSize(requirements.size(), ItemStack.EMPTY);
      ...
      for (int i = 0; i < requirements.size(); i++) {
        ...
        if (requirement.getPossiblePart() instanceof ToolPartItem) {
          ...
          ItemStack toolPartWithMaterial = toolPart.getItemstackWithMaterial(materials.get(TConstruct.random.nextInt(materials.size())));
          stacks.set(i, toolPartWithMaterial);
        }
      }

      return ToolBuildHandler.buildItemFromStacks(stacks, this);
    }
