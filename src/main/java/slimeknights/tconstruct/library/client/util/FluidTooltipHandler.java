...
public class FluidTooltipHandler {
  ...
  private static List<FluidGuiEntry> calcFluidEntries(Fluid fluid) {
    ...
    for (AbstractCastingRecipe recipe : RecipeUtil.getRecipes(manager, RecipeTypes.CASTING_TABLE, AbstractCastingRecipe.class)) {
    ...
    for (AbstractCastingRecipe recipe : RecipeUtil.getRecipes(manager, RecipeTypes.CASTING_BASIN, AbstractCastingRecipe.class)) {
      // no cast, copy amount
      FluidIngredient ingredient = recipe.getFluid();
