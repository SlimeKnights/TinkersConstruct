...
public class MaterialDataProvider extends GenericDataProvider {
  ...
  private MaterialJson convert(IMaterial material) {
    ...
    if (material.getFluid() == Fluids.EMPTY) {
      return new MaterialJson(material.isCraftable(), null, null, color, null, material.getTrait());
    }
    return new MaterialJson(material.isCraftable(), material.getFluid().getRegistryName(), material.getFluidPerUnit(), color, material.getTemperature(), material.getTrait());
  }
