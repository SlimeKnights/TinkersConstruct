...
public class BattleAxe extends AoeToolCore {
  ...
  public NBTTagCompound buildTag(List<Material> materials) {
    ...
    //data.speed *= 1f + 0.1f * (handle.multiplier * handle.miningspeed);
    data.speed *= 0.5f; // slower because AOE
    // no base damage but higher damage potential
    data.attack = (head1.attack + head2.attack) * 3f / 2f;
    //data.attack *= 1f + 0.1f * handle.multiplier * binding.extraQuality;

    /*
    data.durability += head1.durability * (0.2f * head2.extraQuality + 0.2f * binding.extraQuality + 0.1f * handle.multiplier);
    data.durability += head2.durability * (0.2f * head1.extraQuality + 0.2f * binding.extraQuality + 0.1f * handle.multiplier);
    ...
    data.attack += (0.2f + 0.7f * handle.multiplier * binding.extraQuality) * (head1.attack + head2.attack) / 3f;

    data.speed = head1.miningspeed/2f + head2.miningspeed/2f;
    data.speed *= 0.3f + 0.3f * handle.multiplier * binding.extraQuality;
*/
    data.modifiers = 2;
