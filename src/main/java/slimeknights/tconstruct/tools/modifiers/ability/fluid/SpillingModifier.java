    ...
    if (damageDealt > 0 && context.isFullyCharged()) {
      ...
      if (!fluid.isEmpty()) {
        ...
        if (recipe.hasEntityEffects()) {
          ...
          int consumed = recipe.applyToEntity(fluid, modifier.getEffectiveLevel(), FluidEffectContext.builder(living.level()).user(living, player).target(context.getTarget(), context.getLivingTarget()), FluidAction.EXECUTE);
