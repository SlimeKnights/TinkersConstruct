...
public interface ICastingManager extends IRecipeManager {
  ...
  @ZenCodeType.Method
  default void addCompositeCastingRecipe(String name, String inputId, IFluidStack fluidstack, String outputId, int coolingTemperature) {
    name = fixRecipeName(name);
    ResourceLocation id = new ResourceLocation("crafttweaker", name);
    MaterialId inputMaterialId = CRTHelper.getMaterialId(inputId);
    FluidIngredient fluid = FluidIngredient.of(fluidstack.getInternal());
    MaterialId outputMaterialId = CRTHelper.getMaterialId(outputId);
    CompositeCastingRecipe recipe = makeCompositeCastingRecipe(id, inputMaterialId, fluid, outputMaterialId, coolingTemperature);
