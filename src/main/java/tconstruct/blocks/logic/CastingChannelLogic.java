...
//			return;
		...
		TileEntity possibleFaucet = worldObj.getBlockTileEntity(xCoord, yCoord+1, zCoord);
	...
	public void changeOutputs(EntityPlayer player, int side, float hitX, float hitY, float hitZ)
	{
		...
		{
			TileEntity tile = worldObj.getBlockTileEntity(xCoord+toggle.offsetX,yCoord+toggle.offsetY,zCoord+toggle.offsetZ);
...
//				debugLog.clear();
	...
	public void writeCustomNBT (NBTTagCompound tags)
	{
		...
		tags.setCompoundTag("internalTank", nbtTank);
		for(ForgeDirection fdSub: subTanks.keySet())
		{
			NBTTagCompound nbtSubTank = new NBTTagCompound();
			subTanks.get(fdSub).writeToNBT(nbtSubTank);
			tags.setCompoundTag("subTank_"+fdSub.name(), nbtSubTank);
		}
